...
### 2026-04-18 — Добавлена vm-apps-01 с MeTube

- **Тип:** added
- **VM:** vm-apps-01 (новый)
- **Описание:** Создана VM для прикладных сервисов. Развёрнут MeTube (веб-интерфейс для yt-dlp) — сервис для загрузки видео с YouTube и других видеоплатформ. Порт :8081. Параметры загрузки (качество, формат звука) управляются через `.env` файл. Docker volume для хранения загруженных видео. Обновлена конфигурация Claude Code (WebSearch в allowlist).
- **Версии:** `ghcr.io/alexta69/metube:latest`
- **Коммит:** 54ee891
