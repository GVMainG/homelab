...
vm-proxy-02 (192.168.1.51)
   ...
   ├── :81   → NPM Admin UI
   └── :53   → dnsmasq (split-DNS: *.home.loc → 192.168.1.51)
         │
         ▼
...
|---|---|---|---|---|
| `vm-db-02` | 192.168.1.52 | Linux | База данных + менеджмент | PostgreSQL 16, Vaultwarden (latest), pgAdmin 4 (latest) |
| `vm-proxy-02` | 192.168.1.51 | Linux | Reverse proxy + DNS | Nginx Proxy Manager (latest), dnsmasq (split-DNS) |
...
|---|---|---|
| `*.home.loc` | 192.168.1.51 | Wildcard DNS через dnsmasq на vm-proxy-02 |
| `vw.home.loc` | 192.168.1.51 | Vaultwarden (через NPM reverse proxy на vm-db-02:8080) |

## Сервисы и endpoint'ы

| Сервис | VM | Порт | URL | Доступ |
|---|---|---|---|---|
| PostgreSQL 16 | vm-db-02 | 5432 (проброшен на LAN) | `192.168.1.52:5432` | LAN |
| Vaultwarden | vm-db-02 | 8080 | `http://192.168.1.52:8080` | LAN |
| pgAdmin | vm-db-02 | 5050 | `http://192.168.1.52:5050` | LAN |
| Nginx Proxy Manager | vm-proxy-02 | 81 | `http://192.168.1.51:81` | LAN (admin UI) |
...
dnsmasq (vm-proxy-02) ── DNS resolution ──▶ *.home.loc → 192.168.1.51 (NPM)
