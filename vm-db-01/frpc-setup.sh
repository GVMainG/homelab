#!/usr/bin/env bash
# frpc-setup.sh — настройка FRP-клиента на домашнем сервере (vm-db-01).
# Интерактивно спрашивает параметры VPS и локальные сервисы для проброса,
...
if ! command -v docker &>/dev/null; then
    log_error "Docker не установлен. Установите Docker и повторите запуск."
    exit 1
fi
log_info "Docker найден: $(docker --version)"

# ── Шаг 2: Интерактивный ввод параметров VPS ─────────────────────────────────
log_step "Параметры FRP-сервера (VPS)"
echo "  (Эти значения есть в выводе setup.sh на VPS)"
echo ""
...
# ── Шаг 3: Выбор сервисов для проброса ───────────────────────────────────────
log_step "Сервисы для проброса через туннель"
echo "  Для каждого сервиса укажите порт на VPS (remotePort)."
echo "  NPM на VPS будет принимать трафик на этот порт и отдавать в домен."
echo ""

# Массивы для хранения конфигурации прокси
PROXY_NAMES=()
PROXY_LOCAL_PORTS=()
PROXY_REMOTE_PORTS=()

# ── Предустановленные сервисы homelab ────────────────────────────────────────
add_preset() {
    local name="$1"
    local local_port="$2"
    ...
    read -r -p "  Пробросить ${desc} (локальный порт :${local_port})? [Y/n]: " answer
    if [[ "${answer,,}" != "n" ]]; then
        local remote
        remote="$(read_default "    Порт на VPS (remotePort)" "$default_remote")"
        PROXY_NAMES+=("$name")
...
add_preset "vaultwarden" "8080" "18080" "Vaultwarden"
add_preset "pgadmin"     "5050" "15050" "pgAdmin"

# ── Добавление произвольных сервисов ─────────────────────────────────────────
echo "  Добавить другой сервис?"
while true; do
    read -r -p "  [y/N]: " add_more
    if [[ "${add_more,,}" != "y" ]]; then
        break
    fi
    custom_name="$(read_default "  Имя сервиса (латиница, без пробелов)")"
    if [[ -z "$custom_name" ]]; then
        log_warn "Имя не может быть пустым."
        continue
    fi
    custom_local="$(read_default "  Локальный порт")"
    custom_remote="$(read_default "  Порт на VPS (remotePort)")"
...
if [[ ${#PROXY_NAMES[@]} -eq 0 ]]; then
    log_error "Не выбрано ни одного сервиса. Добавьте хотя бы один."
    exit 1
fi
...
if [[ -f "$FRPC_CONFIG" ]]; then
    ...
    read -r -p "  Перезаписать конфигурацию? [y/N]: " answer
    if [[ "${answer,,}" != "y" ]]; then
...
# Не коммитить в git (содержит токен).
...
# ── Шаг 6: Генерация docker-compose.yml для frpc ─────────────────────────────
log_step "Генерация frpc/docker-compose.yml"

cat > "$FRPC_COMPOSE" <<'EOF'
# docker-compose.yml для frpc
# network_mode: host — frpc видит 127.0.0.1 хоста, что позволяет подключаться
# к Vaultwarden, pgAdmin и другим сервисам по их локальным портам напрямую.
services:
  frpc:
...
echo ""
echo -e "  ${YELLOW}Следующий шаг — настройка NPM на VPS:${NC}"
for i in "${!PROXY_NAMES[@]}"; do
    echo -e "    Proxy Host: <домен> → http://127.0.0.1:${PROXY_REMOTE_PORTS[$i]}"
done
echo ""
echo -e "  Статус туннеля: ${CYAN}http://${VPS_HOST}:7500${NC}"
echo ""
